...
def sort_candidates(gallery, query):
    ...
    for i in range(len(gallery)):
        ...
        candidates.append((np.dot(candidate[:-1], query[:-6]), identity))
...
def matching(gallery, dataset):
    ...
    dataset['correct_matching_score'] = 0
    for i in range(len(dataset)):
        ...
        position = find_position(candidates, dataset.loc[i, 'identity'])
        dataset.loc[i, 'query_id_position'] = position
        dataset.loc[i, 'correct_matching_score'] = candidates[position][0]
    return dataset


if __name__ == '__main__':
    df_deep_features = pd.read_csv('./deep_features_cleanup.csv', index_col=[0])
    gallery_deep_features = pd.read_csv('./gallery_deep_features.csv', index_col=[0])
    ...
    df_deep_features.to_csv('./matching.csv')